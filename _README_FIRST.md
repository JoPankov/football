...
- Command-first UX: select a player, pick an action (Move is armed by default and stays armed after a walk), click a highlighted tile. Move highlights every cell a leftover-AP cone-walk can still reach, including occupied squares (amber) and after one prefix turn (1 AP up to 90°, 2 AP for 135°/180°) — clicking a far tile queues that turn if needed, then every step on the cheapest path. Teammate cells can be landed on but not walked through. Opponent cells are dests and through-tiles (treated as empty for the walk). A pending dest wins over selecting a teammate. Right-click an adjacent cell to turn toward it without picking Turn. Backspace pops the last queued action of the selected player, or this side’s last action if they have none / nobody is selected. One cell is often two actions (adjacent empty = move or pass; adjacent occupied = move, pass, contest, or swap; net = shoot and maybe move). Do not revive the old tile-then-chooser without wiring; tests assume command-then-tile.
- A player’s queued actions are an array, not a single slot. `ap_spent` = sum of each plan’s `ap_cost`. Cap is `PLAYER_ACTION_POINTS` (6). Orthogonal steps cost 2, diagonal 3, sprint 2 AP / 3 energy straight or 3 AP / 5 energy diagonally, dribble/tackle/fight 5 energy, pass 1, turn 1 AP up to 90° or 2 AP for 135°/180°. A shot costs every leftover AP. Cap on distinct acting players is `ACTIONS_PER_SIDE` (3).
...
- Move = 1 tile into the **3-cell cone** (facing ± 45°). The Move highlight is every cell a cone-walk can still reach with leftover AP, including occupied dests and after one prefix turn (1 AP up to 90°, 2 AP for 135°/180°). Teammate cells are dests, not through-tiles. Opponent cells are dests and through-tiles. Turns are not inserted mid-walk. Sprint = 2 tiles **straight ahead** of facing (not the cone), 2 AP / 3 energy straight or 3 AP / 5 energy diagonally. Through must be empty; occupied landing is legal and becomes a contest at apply if they stayed. Turn faces any of the other 7 directions: 45°/90° cost 1 AP, 135°/180° cost 2 AP. Pass range = Euclidean radius 5 tile lengths (cell centre to cell centre), not into the rear cone except adjacent cells. Pass dests include occupied cells (teammate or opponent); empty nets and the opponent net are still illegal.
...
- Intercept and shot math use **tile units**, not pixels. `MatchRules.tile_center(cell)` is `Vector2(cell)`. `INTERCEPT_RADIUS = 0.7` tile. Adjacent orthogonal cells sit 1.0 away, so they do not intercept an axis-aligned pass. `TILE_SIZE = 72` is drawing only. Shots use the same intercept lane as passes (`interceptors_for_pass` with dest = opponent net). The keeper in the net does not intercept; they save if the ball arrives. During resolve the ball flies the **whole remaining path** in the wave the pass or shot is performed; intercepts re-check **live** positions after that wave’s movement, so a player who stepped onto the lane in an earlier wave — or in this wave before the ball flew — still gets a throw. Too late if they arrive after.
...
2. **Never two players on one cell.** Dribble and square-fight wins swap; a won tackle steals in place; intercepts refuse occupied landings; a queued move onto occupied becomes that contest at apply time (same-team occupancy is always a square fight). Planning occupancy does not block move/pass dests. Opponent occupancy does not absorb a cone-walk; teammates still do.
