...
- A player’s queued actions are an array, not a single slot. `ap_spent` = sum of each plan’s `ap_cost`. Cap is `PLAYER_ACTION_POINTS` (6). Orthogonal steps cost 2, diagonal 3, sprint 2 (and 3 energy), pass 1, turn 1 AP up to 90° or 2 AP for 135°/180°. A shot costs every leftover AP. Cap on distinct acting players is `ACTIONS_PER_SIDE` (3).
...
- Move = 1 tile into the **3-cell cone** (facing ± 45°). The Move highlight is every empty cell a cone-walk can still reach with leftover AP (no auto-turns). Sprint = 2 tiles **straight ahead** of facing (not the cone), 2 AP and 3 energy, both through and landing empty. Turn faces any of the other 7 directions: 45°/90° cost 1 AP, 135°/180° cost 2 AP. Pass range = Euclidean radius 5 tile lengths (cell centre to cell centre), not into the rear cone except adjacent cells.
